...
import type { SortStep, MergeTreeNode } from '@/types/sortStep'

interface MergeTreePanelProps {
  step: SortStep
}

export function MergeTreePanel({ step }: MergeTreePanelProps) {
...
function TreeNode({ treeNodes, node, activeNodeId }: { treeNodes: MergeTreeNode[], node: MergeTreeNode, activeNodeId?: string }) {
  ...
  return (
    <div className="flex flex-col items-center">
      <motion.div
        layout
        className={`z-10 flex items-center justify-center rounded border px-3 py-1.5 transition-colors ${bgColor}`}
      >
      ...
      {(left || right) && (
        <div className="relative mt-4 flex gap-4 sm:gap-8">
          {/* SVG lines connecting parent to children could be drawn here, but relying on simple flex layout for now */}
